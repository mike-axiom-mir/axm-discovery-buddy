...
Turn a saved discovery index into deterministic capability candidates without executing or selecting them:
...
The cross-repository bridge is static evidence consumption. Discovery Buddy does not import, execute, install, or make another AXM repository a runtime dependency. Capability queries return evidence-only candidates and grant no execution, automatic selection, installation, merge, or CANON authority.
...
A repository is omitted unless it carries an explicit `.axm/discovery-public.json` marker using schema `axm.discovery-public/v1` with `public: true`. Public output strips local paths and Git work-state fields and keeps only bounded whitelisted discovery metadata.
...
- automatic provider execution, ranking, adoption, or capability compatibility proof;
- measured large-workspace performance or scale guarantees.

A declared capability or a query match is discovery evidence, not runtime proof, quality, authority, or CANON. The existing Organ Beacon remains a separate proposal-only evidence transport lane.
...
Focused capability-query suite:
